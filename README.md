...
순수 Spring MVC 프로젝트에 Maven 로컬 빌드로 연동하여 **실제 서비스에 적용 중**입니다.
...
Spring은 이 메서드들만 알면 된다. 내부 인덱스 구현이 바뀌어도 서비스 코드는 변경이 없다.

| 메서드 | 용도 |
|--------|------|
| `search(lat, lng, radiusKm)` | 반경 검색 — HIT/MISS 판단 후 반환 |
