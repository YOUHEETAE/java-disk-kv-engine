...
어느 단계에서 예외가 발생하든 임시 파일을 삭제하고 기존 파일로 서비스를 계속한다.

```
단계 4(기존 파일 닫기) 이후 실패:
  기존 파일은 디스크에 존재하지만 닫힌 상태
  → 임시 파일 삭제 + 기존 파일 재오픈 → 서비스 계속

성공/실패 시나리오:
  성공: 임시 파일 완성 → atomic move → 기존 파일 교체 ✅
  실패: 어느 단계든 예외 → 임시 파일 삭제 → 기존 파일 유지 → rebuild 재시도
```

Windows에서는 열린 파일을 이동할 수 없으므로 기존 파일을 먼저 닫은 후 move를 수행한다.
