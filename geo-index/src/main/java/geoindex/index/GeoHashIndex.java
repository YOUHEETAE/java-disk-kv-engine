...
/**
 * 좌표를 격자 번호로 바꾸는 계층. 위경도를 축당 15비트로 양자화한 뒤 비트를 교차시켜
 * Morton(Z-곡선) 정수를 만들고, 그 값을 그대로 pageId 로 쓴다.
 *
 * 저장하는 것이 없다. 좌표 → 번호는 순수 계산이고, 어떤 번호에 무엇이 들어 있는지는
 * 이 클래스가 모른다. 그래서 색인을 파일에 쓰거나 재구축할 필요가 없다.
 *
 * 실제 역할은 후보를 줄이는 것이 아니라 <b>캐시 키를 주는 것</b>이다. 같은 지역을 물으면
 * 같은 pageId 가 나오므로, 인덱스 단위 · 캐시 단위 · 무효화 단위가 모두 pageId 하나로
 * 맞춰진다. 반경을 덮는 격자에는 아무도 쓴 적 없는 칸이 대량으로 섞인다.
 */
