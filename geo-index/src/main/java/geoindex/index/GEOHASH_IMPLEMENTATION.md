...
Z-곡선(Morton)은 격자 경계에서 큰 점프가 생긴다. Hilbert 곡선은 그 점프가 없어
공간 인접성이 1차원에서 더 잘 보존되므로, 구현해서 비교했다.

채택하지 않은 이유는 두 가지다.

- **이점이 나오는 지점까지 구현하지 않았다.** Hilbert의 실질 이득은 조회 범위를
  연속 구간 몇 개로 병합하는 Multi-Interval Query에서 나온다. 구현한 것은 격자
  열거까지였고, 그 상태에서는 곡선을 바꾼 효과가 제한적이다.
- **비교 조건이 동일하지 않았다.** 결정적인 것은 페이지 입도다. Hilbert 쪽은
  `MAX_PAGE = 10000`으로 힐버트값 약 10만 개를 페이지 하나에 묶었고, GeoHash는
  Morton 값 자체를 pageId로 썼다. "pageId 13개 vs 187개"는 곡선의 차이가 아니라
  **페이지 크기의 차이**였다. 좌표계(전지구 vs 한국 박스)와 격자 해상도도 달랐다.
  그 수치를 근거로 쓸 수 없다고 판단해 비교표를 걷어냈다.

현재 규모에서는 파일 전체가 OS 페이지 캐시에 올라가 seek 거리가 응답 시간에
드러나지 않는다. 곡선 선택보다 **격자 크기와 파일 배치 순서**가 먼저 영향을 준다고
보고 Z-곡선을 유지했으며, Hilbert 구현은 제거했다(코드는 git 이력에 남아 있다).
