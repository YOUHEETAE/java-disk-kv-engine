...
public class GeoHash {
    ...
    // Morton → latBits/lngBits 분리
    public static long[] deinterleave(long morton) {
        long lngBits = 0;
        long latBits = 0;
        for (int i = BITS - 1; i >= 0; i--) {
            lngBits = (lngBits << 1) | ((morton >> (i * 2 + 1)) & 1);
            latBits = (latBits << 1) | ((morton >> (i * 2)) & 1);
        }
        return new long[]{lngBits, latBits};
    }
